...
# Panel category metadata (populated by settings UI adapters when available)
...
def get_all_params(params: Optional[Params] = None, locale: Optional[str] = None) -> Dict[str, Any]:
    """Get all readable parameters

    Args:
        params: Params instance (creates new one if None)
        locale: Portal UI locale ('en' or 'zh-CHS')
...
def _build_param_entry(
    ...
    try:
        from bluepilot.backend.params.param_labels import get_param_label
        label = get_param_label(key, locale)
        if label.get("title"):
            entry["title"] = label["title"]
        if label.get("description"):
            entry["description"] = label["description"]
        if label.get("category_label"):
            entry["category"] = label["category_label"]
        if label.get("panel"):
            entry["panel"] = label["panel"]
        if label.get("section"):
            entry["section"] = label["section"]
    except Exception:
        logger.debug("Failed to attach localized label for param %s", key)
