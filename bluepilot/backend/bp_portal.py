...
from bluepilot.backend.network import (
    is_onroad, should_server_run, get_portal_port, get_portal_language, resolve_bind_port,
    teardown_port_redirect, get_wifi_ip, get_all_wifi_ips, get_connection_type,
)
...
class WebRoutesHandler(BaseHTTPRequestHandler):
    ...
    def send_spa_index_response(self):
        """Serve SPA index.html with locale bootstrap for runtime i18n."""
        index_path = WEBAPP_DIR / 'index.html'
        if not index_path.exists():
            self.send_json_response({'error': 'Web app not found'}, 404)
            return

        locale = get_portal_language(params)
        html = index_path.read_text(encoding='utf-8')

        inject = (
            f'    <script>window.__PORTAL_LOCALE__="{locale}";</script>\n'
            '    <script src="/portal-logs.js"></script>\n'
            '    <script src="/portal-panda-version.js"></script>\n'
        )
        if locale == 'zh-CHS':
            html = html.replace('<html lang="en">', '<html lang="zh-CN">', 1)
            inject += '    <script src="/portal-i18n.js"></script>\n'

        module_marker = '<script type="module" crossorigin src="/assets/index-'
        if module_marker in html:
            html = html.replace(module_marker, inject + module_marker, 1)
        elif '</head>' in html:
        ...
        try:
            ...
            if path == '/api/health':
                ...
                try:
                    ...
                    # Get branch name from GitBranch param (falls back to git)
                    try:
                        branch = None
                        branch_bytes = params.get("GitBranch")
                        if branch_bytes:
                            if isinstance(branch_bytes, bytes):
                                branch = branch_bytes.decode('utf-8').strip()
                            elif isinstance(branch_bytes, str):
                                branch = branch_bytes.strip()
                        ...
                        device_info['bp_version'] = branch if branch else None
                    except Exception as e:
                    ...
                    # Get Agnos version from /VERSION file (comma devices)
                    try:
                        if os.path.exists('/VERSION'):
                            with open('/VERSION', 'r') as f:
                                agnos_version = f.read().strip()
                                device_info['agnos_version'] = agnos_version if agnos_version else None
                        else:
                            device_info['agnos_version'] = None
                    except Exception as e:
                        logger.debug(f"Error reading Agnos version: {e}")
                        device_info['agnos_version'] = None

                    try:
                        from bluepilot.backend.system.device_info import get_panda_version
                        device_info['panda_version'] = get_panda_version()
                    except Exception as e:
                        logger.debug(f"Error reading Panda version: {e}")
                        device_info['panda_version'] = None
            ...
            elif path == '/api/param-labels':
                try:
                    from bluepilot.backend.params.param_labels import get_param_labels_map
                    locale = get_portal_language(params)
                    self.send_json_response({
                        'success': True,
                        'locale': locale,
                        'labels': get_param_labels_map(locale),
                    })
                except Exception as e:
                    logger.exception("Error getting param labels")
                ...
                try:
                    locale = get_portal_language(params)
                    all_params = get_all_params(params, locale=locale)
                    self.send_json_response({
                        'success': True,
                        'locale': locale,
                        'params': all_params
                ...
                try:
                    from bluepilot.backend.params.settings_ui_panels import list_panels
                    panels = list_panels()
                    self.send_json_response({
                        'success': True,
                        'panels': panels
                ...
                try:
                    from bluepilot.backend.params.settings_ui_panels import get_panel
...
def cleanup_on_shutdown():
    ...
    try:
        ...
        logger.debug(f"Error tearing down port redirect: {e}")
...
def main():
    ...
    public_port = get_portal_port(params)

    logger.info(f"Starting BluePilot Web Routes Server on public port {public_port}")
    ...
    try:
        ...
        if server_state.wait_for_websocket(timeout=2.0):
            ...
            broadcaster_instance = WebSocketBroadcaster(http_fallback_port=public_port)
            server_state.set_broadcaster(broadcaster_instance)
    ...
    redirect_ips = [ip for _, ip in all_wifi_ips] if all_wifi_ips else None
    port, redirect_active = resolve_bind_port(public_port, redirect_ips)
    if redirect_active:
        logger.info(f"Port redirect active: public :{public_port} -> bind :{port}")
    elif public_port != port:
        logger.warning(f"Public port {public_port} differs from bind port {port}")
    ...
    for attempt in range(max_retries):
        try:
            ...
            logger.info(f"Successfully bound to {bind_address}:{port} (public port {public_port})")
            break
        except OSError as e:
    ...
    try:
        logger.info(f"Web server starting on {bind_address}:{port} (public port {public_port})")
