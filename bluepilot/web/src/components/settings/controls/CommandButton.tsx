...
export function CommandButton({ control, disabled, disabledReason }: CommandButtonProps) {
  ...
  const handleSetAmapApiKey = () => {
    setInputConfig({
      title: 'Amap API Key',
      message: 'Enter your Gaode/Amap API key for the onroad driving map.<br>Leave empty to remove the key.',
  ...
  const isDangerAction = useMemo(() => {
    ...
    if (control.button_text && dangerButtonTexts.includes(control.button_text.toUpperCase())) {
      return true
    }
    // SSH key removal
    if (sshKeysState?.has_keys) {
