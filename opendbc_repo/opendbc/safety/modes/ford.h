...
static bool ford_get_quality_flag_valid(const CANPacket_t *msg) {
  ...
  if (msg->addr == FORD_BrakeSysFeatures) {
    ...
    // BluePilot: at standstill the RCM yaw quality can legitimately drop (no motion to
    // validate). The yaw-sourced angle_meas is only consumed above angle_error_min_speed
    // (10 m/s), so bypassing the quality check while stopped is safe and prevents a spurious
    // safetyRxChecksInvalid -> controls_allowed=false -> "Controls Mismatch" NO_ENTRY during
    // stop-and-go resume (the ACC icon lights then gets cancelled for ~3s).
    valid = !vehicle_moving || (((msg->data[6] >> 4) & 0x3U) == 0x3U);  // VehYawWActl_D_Qf
  } else if (msg->addr == FORD_SteeringPinion_Data) {
    // Same standstill bypass for the pinion compensation-estimate quality.
    valid = !vehicle_moving || (((msg->data[5] >> 2) & 0x3U) == 0x3U);  // StePinCompAnEst_D_Qf (3=OK)
  } else {
  }
  return valid;
}
...
static const AngleSteeringParams ford_pinion_geometry[FORD_PINION_GEOMETRY_COUNT + 1U] = {
  ...
  {.slip_factor = -0.00045454798f, .steer_ratio = 17.0f, .wheelbase = 3.690f},    // 8: FORD_F_150_MK14
