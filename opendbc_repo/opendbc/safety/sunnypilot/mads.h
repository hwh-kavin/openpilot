...
static bool heartbeat_engaged_mads_prev = false;
...
inline void m_mads_state_init(void) {
  ...
  heartbeat_engaged_mads_prev = false;
}
...
static inline bool m_mads_can_heartbeat_reengage_lateral(void) {
  if (!m_mads_state.system_enabled || controls_allowed_lateral || m_mads_state.braking.current) {
    return false;
  }

  if (!m_mads_state.acc_main.current) {
    return false;
  }

  const DisengageReason reason = m_mads_state.current_disengage.active_reason;
  return (reason == MADS_DISENGAGE_REASON_NONE) ||
         (reason == MADS_DISENGAGE_REASON_HEARTBEAT_ENGAGED_MISMATCH);
}

static inline void m_mads_reengage_lateral_from_heartbeat(void) {
  controls_allowed_lateral = true;
  m_mads_state.controls_requested_lateral = false;
  m_mads_state.current_disengage.active_reason = MADS_DISENGAGE_REASON_NONE;
  m_mads_state.current_disengage.pending_reasons = MADS_DISENGAGE_REASON_NONE;
...
inline void mads_heartbeat_engaged_check(void) {
  // Python confirms MADS via heartbeat after acc_main may already be active. Re-grant lateral
  // without requiring another acc_main rising edge (fixes heartbeat/acc_main engage race).
  if (heartbeat_engaged_mads && !heartbeat_engaged_mads_prev) {
    if (m_mads_can_heartbeat_reengage_lateral()) {
      m_mads_reengage_lateral_from_heartbeat();
    }
  }
  ...
  heartbeat_engaged_mads_prev = heartbeat_engaged_mads;
}
