...
本分支从 sunnypilot 的 master 分支 fork 而来, 260427是fork日期；L2辅助驾驶，使用请郑重！

  1、修改panda，适配C3；
  
  2、优化了纵向，将原车纵向和OP视觉纵向融合，弯道减速、高速状态远距离优先选择OP预刹车；

  3、增加了驾驶员接管方向盘大角度转向后，回正方向盘，存在抢方向盘的问题；
  
  4、优化跟车起步响应；
  
  5、优化刹车，允许超过设定速度，优先使用油门减速，再启用EPS刹车；

  6、增加了驾驶员监控模型使能；
  
  7、增加 CarLife Companion 分屏地图：手机 APP 经 UDP 推送地图截图（:8889）与车道/灯态/限速等 JSON（:8888），车机直接显示，无需高德密钥；
  
  8、从bp中获取了网页服务器，手机浏览器可以访问openpilot，配置参数；
  
  9、横向增加了跟随前方弯道最大曲率，动态改变横向曲率提前量；理论上能过更大弯道；
  
  10、修改了panda的部分横向限制；横向最大步径角度能提高一点；

  11、从bp中获取了盲点监测系统 显示渐变红，更醒目也更美观；

  12、2026年8月11日，同步了最新模型V2构架

  13、支持手机地图镜像显示在OP界面，行车界面触摸会分屏显示地图，需要配合手机镜像软件；
