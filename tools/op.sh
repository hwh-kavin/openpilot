...
function op_check_git() {
  ...
  for name in panda opendbc_repo msgq_repo rednose_repo teleoprtc_repo tinygrad_repo sunnypilot/neural_network_data; do
    if [[ ! -d "$OPENPILOT_ROOT/$name" ]]; then
      echo -e " ↳ [${RED}✗${NC}] required directory $name not found!"
      return 1
    fi
  done
...
function op_check_os() {
  ...
  if [[ "$OSTYPE" == "linux-gnu"* ]]; then
    ...
    if [ -f "/AGNOS" ]; then
      echo -e " ↳ [${GREEN}✔${NC}] AGNOS $(cat /VERSION 2>/dev/null) detected."
...
function op_setup() {
  ...
  echo "Skipping git submodules (vendored in repo)..."
  st="$(date +%s)"
  et="$(date +%s)"
