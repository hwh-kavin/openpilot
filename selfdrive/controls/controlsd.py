...
class Controls(ControlsExt):
  ...
  def state_control(self):
    ...
    # HTD: pause lateral when driver makes a large manual turn
    model_steer_rad = self.VM.get_steer_from_curvature(-model_v2.action.desiredCurvature, CS.vEgo, lp.roll)
    model_desired_angle_deg = math.degrees(model_steer_rad) + lp.angleOffsetDeg
    htd_allowed, self.htd_state = self.htd.update(
        _lat_active,
        CS.steeringAngleDeg,
        CS.vEgo,
        CS.steeringPressed,
        model_desired_angle_deg,
        DT_CTRL,
    )
