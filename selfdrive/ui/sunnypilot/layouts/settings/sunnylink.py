...
from openpilot.selfdrive.ui.sunnypilot.widgets.portal_qr import PortalQrPanel
...
class SunnylinkLayout(Widget):
  ...
  def _initialize_items(self):
    self._portal_toggle = toggle_item_sp(
      title=tr("BluePilot Portal Service"),
      description=tr("BluePilot Portal is a web-based interface for viewing routes, logs, system metrics, and device settings. "
                     "Connect to your comma device locally via its IP address (default port 80)."),
      param="EnableCopyparty",
      callback=self._on_portal_toggle,
    )

    self._portal_qr = PortalQrPanel()
    self._portal_qr.set_visible(ui_state.params.get_bool("EnableCopyparty"))
  ...
  def _on_portal_toggle(self, state: bool):
    self._update_portal_qr_visibility(state)

  def _update_portal_qr_visibility(self, enabled: bool | None = None):
    if enabled is None:
      enabled = ui_state.params.get_bool("EnableCopyparty")
    self._portal_qr.set_visible(enabled)
    if enabled:
      self._portal_qr.regenerate_qr_code()
  ...
  def _update_state(self):
    super()._update_state()
    self._update_portal_qr_visibility()
