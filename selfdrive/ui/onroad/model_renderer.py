...
class VehicleDynamicsDisplay:
  def __init__(self):
    self._font = rl.load_font("fonts/Roboto-Medium.ttf", 20)
    self._bg_color = rl.Color(0, 0, 0, 128)  # 50% transparent black
    self._text_color = rl.Color(255, 255, 255, 192)  # 75% transparent white

  def draw(self, rect, sm):
    if not (sm.alive.get('carState', False) and sm.alive.get('controlsState', False)):
      return
    ...
    # Draw background
    bg_height = len(info_lines) * 22 + 20
    bg_rect = rl.Rectangle(
      rect.x + 10,
      rect.y + rect.height - bg_height - 10,
      300,
      bg_height
    )
    rl.draw_rectangle_rec(bg_rect, self._bg_color)

    # Draw text
    start_y = rect.y + rect.height - bg_height
    for i, line in enumerate(info_lines):
      rl.draw_text_ex(
        self._font,
        line,
        rl.Vector2(rect.x + 20, start_y + i * 22 + 10),
        20,
        0,
        self._text_color
      )
...
class ModelRenderer:
  def __init__(self):
    ...
    self._dynamics_display = VehicleDynamicsDisplay()
  ...
  def _draw_vehicle_dynamics(self):
    """Draw vehicle dynamics information at bottom of screen"""
    if not (hasattr(self, 'sm') and self.sm.alive.get('carState', False) and self.sm.alive.get('controlsState', False)):
      return

    car_state = self.sm['carState']
    controls_state = self.sm['controlsState']

    # Format display info in two columns
    info_lines = [
      (f"预测曲率: {controls_state.curvature:.4f} m⁻¹",
       f"方向盘角度: {car_state.steeringAngleDeg:.1f}°"),
      (f"期望曲率: {controls_state.desiredCurvature:.4f} m⁻¹",
    ...
    bg_width = col_width * 2 + padding * 3

    # Draw text in two columns with transparent background
    start_y = self._rect.y + self._rect.height - bg_height - 10 + padding
    blue_color = rl.Color(100, 149, 237, 166)  # 65% transparency cornflower blue
    for i, (left_text, right_text) in enumerate(info_lines):
      # Left column
      rl.draw_text_ex(
        self._font_medium,
        left_text,
        rl.Vector2(self._rect.x + 20, start_y + i * line_height),
        20,
        0,
        blue_color
      )
      ...
      rl.draw_text_ex(
        self._font_medium,
        right_text,
        rl.Vector2(self._rect.x + 20 + col_width + padding, start_y + i * line_height),
        20,
        0,
        blue_color
      )
