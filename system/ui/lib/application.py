...
_DEFAULT_FPS = int(os.getenv("FPS", 20))
...
class GuiApplication(GuiApplicationExt):
  def __init__(self, width: int | None = None, height: int | None = None):
    ...
    self._font_remap: dict[int, FontWeight] = {}
  ...
  def font(self, font_weight: FontWeight = FontWeight.NORMAL) -> rl.Font:
    if font_weight not in self._fonts:
      ...
      with as_file(FONT_DIR) as fspath:
        ...
        if not fnt_path.exists() and multilang.requires_unifont() and font_weight != FontWeight.UNIFONT:
